...
error_reporting(0);
	...
	$insert=mysqli_query($con,"UPDATE `info_cad` SET `cstatus`='$cstatus',`call_date`='$in_time',`d_name`='$d_name',`inter_date`='$select_date' WHERE id='$id'");

		if($insert){
		
			$msg="<p class='alert alert-success alert-rounded'>Successfully Updated ! </p>";
			
				echo '<script>window.location.href = "info_cad";</script>';
		}
